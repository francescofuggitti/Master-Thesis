...
However, the representation of an \rcon\xspace, as in Definition \ref{def:rcon}, has two main drawbacks:
\begin{itemize}
...
%\end{definition}
 ...
 In Section \ref{sec:janus-implementation}, we will see the implementation of the Janus algorithm for computing the \emph{interestingness degree} of traces in real-world event logs, using \rcon s declared as in \ref{eq:new-constraint}.
...
Let us consider the \rcon\xspace $\Psi = (False \lAND a \lAND False) \lOR (\Yesterday b \lAND (True \lAND a) \lAND True) \lOR (True \lAND (True \lAND a) \lAND \Diamond c)$. The corresponding sep.aut.set $\automaton^{\blacktriangleleft \blacktriangledown \blacktriangleright}$ for each triple in $\Psi$ is depicted in Table \ref{tab:sep-aut-set}.
\begin{table}
\centering
...
\begin{tabular}{ c  c  c }
\hline
$\automaton^{\blacktriangleleft} = \automaton_{\Yesterday b}$ & $\automaton^{\blacktriangledown} = \automaton_{(\true \lAND a)}$ & $\automaton^{\blacktriangleright} = \automaton_{\true}$\\
\hline
...
\hline
$\automaton^{\blacktriangleleft} = \automaton_{\true}$ & $\automaton^{\blacktriangledown} = \automaton_{(\true \lAND a)}$ & $\automaton^{\blacktriangleright} = \automaton_{\Diamond c}$\\
\hline
