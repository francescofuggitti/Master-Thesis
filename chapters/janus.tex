...
\hline
\raisebox{.4\height}{\includegraphics[width=0.1\textwidth, height=3.5em]{images/automa4-true.eps}}
&
\raisebox{.4\height}{\includegraphics[width=0.1\textwidth, height=3.5em]{images/automa4-true.eps}}
&
\raisebox{.07\height}{\includegraphics[width=0.3\textwidth]{images/example1-output.eps}}\\
\hline
\end{tabular}
\end{center}
\end{example}
...
To begin with, the main goal of \janus, as stated at the beginning, is to compute the \textit{interrestingness degree} of traces on event log. As a consequence, it also provides I/O facilities for three different event log formats, namely simple \textit{.txt} files, \textit{.csv} files and \textit{.xes} files for real-world event logs.
Moreover, it requires Python>=3.6 and has the following dependencies:
\begin{itemize}
...
\end{itemize} 
The \janus software project is open-source and available to download on GitHub\footnote{https://github.com/Francesco17/janus}.
...
Every specific method parses the event log. In particular, at line \ref{line:load-xes}, the \texttt{load\_xes} method takes advantage of the OpyenXES library APIs using its parser and classifier. In Section \ref{sec:janus-main}, we will look at how an \texttt{InputHandler} object can be instantiated.
\subsection{Automata}
